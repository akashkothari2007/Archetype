...
export async function waitJob(id:string,onProgress:(job:Job)=>void):Promise<any>{for(;;){const j=await api<Job>('/jobs/'+id);onProgress(j);if(j.state==='done')return j.result;if(j.state==='error'||j.state==='cancelled')throw new Error(j.error||j.message);await new Promise(r=>setTimeout(r,350));}}
