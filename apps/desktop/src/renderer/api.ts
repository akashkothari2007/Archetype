...
export async function api<T=any>(path:string,body?:unknown,method?:string):Promise<T>{const response=await fetch(base+path,{method:method||(body?'POST':'GET'),cache:'no-store',headers:body instanceof FormData?undefined:{'Content-Type':'application/json'},body:body instanceof FormData?body:body?JSON.stringify(body):undefined});if(!response.ok){const e=await response.json().catch(()=>({detail:response.statusText}));throw new Error(typeof e.detail==='string'?e.detail:JSON.stringify(e.detail))}return response.json()}
export async function waitJob(id:string,onProgress:(job:Job)=>void,timeoutMs?:number):Promise<any>{const started=Date.now();for(;;){const j=await api<Job>('/jobs/'+id);onProgress(j);if(j.state==='done')return j.result;if(j.state==='error'||j.state==='cancelled')throw new Error(j.error||j.message);if(timeoutMs&&Date.now()-started>timeoutMs){void api('/jobs/'+id+'/cancel',{}).catch(()=>undefined);throw new Error('This is taking longer than expected. The request was stopped; please try again.')}await new Promise(r=>setTimeout(r,120));}}
