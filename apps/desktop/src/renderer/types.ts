...
export type Check = {id:string;rule_id:string;entity_id:string;entity_ids:string[];status:'pass'|'fail'|'cannot_verify'|'quarantined';metric:string;actual:number|null;required:number;unit:string;message:string;reason?:string;reliability_reason?:string;source_doc:string;source_page?:number;source_text:string};
