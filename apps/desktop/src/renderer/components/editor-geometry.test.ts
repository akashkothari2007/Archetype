...
describe('floor geometry and asset placement', () => {
  ...
  it('exposes a full default furniture catalog with 3D models', () => {
    ...
    expect(catalogAsset('SchoolDesk_01')?.label).toBe('Student desk')
    expect(catalogAsset('SchoolChair_01')?.label).toBe('Student chair')
    expect(catalogAsset('metal_office_desk')?.label).toBe('Large desk')
    expect(catalogAsset('blackboard')).toMatchObject({ label: 'Blackboard', procedural: 'blackboard', preview: 'assets/blackboard/preview.svg' })
