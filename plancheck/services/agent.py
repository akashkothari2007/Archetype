...
- finish: rename rooms, change materials (plaster/oak/brick/marble/tile/concrete), change time/sun/season, or furnish rooms with catalog furniture
...
def respond_new(
    ...
    **kwargs: Any,
    ...
    if is_furnish_request(prompt) and intent not in {"geometry", "repair"}:
        intent = "finish"
        log.info("agent.override furnish_request forcing intent=finish")
    ...
    if intent == "repair":
        ...
        if is_furnish_request(prompt):
            result = fallback_furnish(building, prompt, selected_ids, floor_id)
        else:
